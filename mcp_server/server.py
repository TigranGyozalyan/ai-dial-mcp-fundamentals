...
@mcp.tool(
    name='search_user',
    description='Search user by name, surname, email or gender',
)
async def search_user(name: str | None = None, surname: str | None = None, email: str | None = None, gender: str | None = None) -> str:
...
@mcp.tool(
    name='update_user',
    description='Update user information by their ID to provided fields'
)
async def update_user(user_id: int, user: UserUpdate) -> str:
    return await user_client.update_user(user_id=user_id, user_update_model=user)
...
@mcp.resource(
    ...
    mime_type='image/png',
    description='Retrieve image of user service flow containing swagger endpoints'
)
def get_flow_diagram() -> bytes:
    return open(Path(__file__).parent / 'flow.png', 'rb').read()
...
@mcp.prompt(
    ...
    description='Prompt describing instructions for searching users'
)
def search_query_prompt() -> str:
    return """
...
@mcp.prompt(
    name='user_create_prompt',
    description='Prompt describing instructions for creating users'
)
def user_create_prompt() -> str:
     return """
