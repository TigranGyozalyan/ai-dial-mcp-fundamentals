...
class DialClient:
    ...
    async def _call_tools(self, ai_message: Message, messages: list[Message]):
        ...
        for tool_call in ai_message.tool_calls:
            ...
            try:
                reply = await self.mcp_client.call_tool(tool_name=tool_name, tool_args=json.loads(tool_args))
                messages.append(Message(role=Role.TOOL, tool_call_id= tool_call_id, content=str(reply)))
            except Exception as e:
                print(f'Error calling tool: {tool_call_id}: {e}')
                messages.append(Message(role=Role.TOOL, content=f'Error calling tool: {tool_call_id}: {e}', tool_call_id=tool_call_id))